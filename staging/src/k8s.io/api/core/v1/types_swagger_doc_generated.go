...
var map_TopologySpreadConstraint = map[string]string{
	...
	"topologyKey":       "TopologyKey is the key of node labels. Nodes that have a label with this key and identical values are considered to be in the same topology. We consider each <key, value> as a \"bucket\", and try to put balanced number of pods into each bucket. We define a domain as a particular instance of a topology. Also, we define eligible domain as a domain that matches the node selector. e.g. If TopologyKey is \"kubernetes.io/hostname\", each Node is a domain of that topology. And, if TopologyKey is \"topology.kubernetes.io/zone\", a domain of that topology is formed by Nodes in a zone. It's a required field.",
	...
	"minDomains":        "MinDomains indicates a minimum number of eligible domains. When the number of domains with matching topology keys is less than `minDomains`, Pod Topology Spread treats \"global minimum\" as 0. As a result, when the number of eligible domains is less than `minDomains`, scheduler won't schedule more than maxSkew Pods to those domains. The default value is 1. Valid values are integers greater than 0. When value is different from 1, WhenUnsatisfiable must be DoNotSchedule.\n\nFor example, in a 3-zone cluster, MaxSkew is set to 2, MinDomains is set to 5 and pods with the same labelSelector spread as 2/2/2: ",
}
