...
func TestPodAffinityScoring(t *testing.T) {
	...
	topologyValue := "topologyvalue1"
	topologyValue2 := "topologyvalue2"
	topologyValue3 := "topologyvalue3"
	topologyValue4 := "topologyvalue4"
	topologyValue5 := "topologyvalue5"
	tests := []struct {
		name         string
	...
	}{
		{
			...
			nodes: []*v1.Node{
				...
				st.MakeNode().Name("node4").Label(topologyKey, topologyValue4).Obj(),
				st.MakeNode().Name("node5").Label(topologyKey, topologyValue5).Obj(),
				st.MakeNode().Name("node6").Label(topologyKey, topologyValue).Obj(),
				st.MakeNode().Name("node7").Label(topologyKey, topologyValue2).Obj(),
				st.MakeNode().Name("node8").Label(topologyKey, topologyValue3).Obj(),
				st.MakeNode().Name("node9").Label(topologyKey, topologyValue4).Obj(),
				st.MakeNode().Name("node10").Label(topologyKey, topologyValue5).Obj(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = wait.Poll(pollInterval, wait.ForeverTestTimeout, podScheduledIn(testCtx.ClientSet, pod.Namespace, pod.Name, tt.expectedNodeName))
			if err != nil {
