...
func SetDefaults_Pod(obj *v1.Pod) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.MinDomainsInPodTopologySpread) {
		for i := range obj.Spec.TopologySpreadConstraints {
			if obj.Spec.TopologySpreadConstraints[i].MinDomains == nil {
				// default MinDomains is 1
				obj.Spec.TopologySpreadConstraints[i].MinDomains = pointer.Int32(1)
			}
		}
	}
}
