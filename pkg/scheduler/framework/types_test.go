...
func Test_newAffinityTerm(t *testing.T) {
	possibleBarLabelValues := []string{"hoge", "fuga", "a"}
	barLabelSelector := &metav1.LabelSelector{
		MatchExpressions: []metav1.LabelSelectorRequirement{
			{
				Key:      "bar",
				Operator: metav1.LabelSelectorOpIn,
				Values:   possibleBarLabelValues,
			},
		},
	}
	barSelector, err := metav1.LabelSelectorAsSelector(barLabelSelector)
	if err != nil {
		t.Fatalf("unexpected error from LabelSelectorAsSelector(): %v", err)
	}

	r, err := labels.NewRequirement("bar", selection.In, possibleBarLabelValues)
	if err != nil {
		t.Fatalf("unexpected error from NewRequirement(): %v", err)
	}
	mergedSelector := labels.SelectorFromSet(labels.Set{"bar": "a"}).Add(*r)
	...
	tests := []struct {
		...
		{
			name: "matchLabelKeys ANDed with LabelSelector when LabelSelector isn't empty (feature flag: enabled)",
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Labels: map[string]string{"foo": "hoge", "bar": "a"}, // MatchLabelKeys will obtain "a"
				},
				Spec: v1.PodSpec{
					Affinity: &v1.Affinity{
						PodAffinity: &v1.PodAffinity{
							RequiredDuringSchedulingIgnoredDuringExecution: []v1.PodAffinityTerm{
								{
									LabelSelector:     barLabelSelector,
									TopologyKey:       "region",
		...
		{
			name: "matchLabelKeys ANDed with LabelSelector when LabelSelector is empty (feature flag: enabled)",
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Labels: map[string]string{"foo": "hoge", "bar": "a"}, // MatchLabelKeys will obtain "a"
				},
			...
			want: &AffinityTerm{
				TopologyKey:       "region",
				Selector:          labels.SelectorFromSet(labels.Set{"bar": "a"}),
				Namespaces:        sets.New("default"),
				NamespaceSelector: labels.Everything(),
			},
		},
		{
			name: "matchLabelKeys is ignored when keys in matchlabelKeys doesn't exist in pod's label (feature flag: enabled)",
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{}, // no labels
				Spec: v1.PodSpec{
					Affinity: &v1.Affinity{
						PodAffinity: &v1.PodAffinity{
							RequiredDuringSchedulingIgnoredDuringExecution: []v1.PodAffinityTerm{
			...
			want: &AffinityTerm{
				TopologyKey:       "region",
				Selector:          barSelector, // no change in selector.
				Namespaces:        sets.New("default"),
				NamespaceSelector: labels.Everything(),
			},
		},
		{
			...
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Labels: map[string]string{"foo": "hoge", "bar": "a"},
				},
				Spec: v1.PodSpec{
					Affinity: &v1.Affinity{
						PodAffinity: &v1.PodAffinity{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := newAffinityTerm(tt.pod, tt.term, tt.enableMatchLabelKeys)
			if err != nil {
				t.Fatalf("unexpected error is returned from newAffinityTerm: %v", err)
			}
			if d := cmp.Diff(tt.want, got); d != "" {
				t.Fatalf("unexpected result from newAffinityTerm() (-want, +got): %s", d)
			}
		})
	}
}
