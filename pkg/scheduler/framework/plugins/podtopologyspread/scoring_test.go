...
func BenchmarkTestPodTopologySpreadScore(b *testing.B) {
	...
	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			...
			state := framework.NewCycleState()
			pl := plugintesting.SetupPlugin(b, frameworkruntime.FactoryAdapter(feature.Features{}, New), &config.PodTopologySpreadArgs{DefaultingType: config.ListDefaulting}, cache.NewSnapshot(existingPods, allNodes))
			p := pl.(*PodTopologySpread)
