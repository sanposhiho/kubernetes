...
func (s *preFilterState) Clone() framework.StateData {
	...
	copy := preFilterState{
		...
		// The number of domains should not be changed by AddPod/RemovePod methods on PreFilter Extensions.
...
func (pl *PodTopologySpread) calPreFilterState(ctx context.Context, pod *v1.Pod) (*preFilterState, error) {
	...
	s := preFilterState{
		...
		TpKeyToDomainsNum:    make(map[string]int, len(constraints)),
	...
	for _, tpCounts := range tpCountsByNode {
		for tp, count := range tpCounts {
			if _, ok := s.TpPairToMatchNum[tp]; !ok {
