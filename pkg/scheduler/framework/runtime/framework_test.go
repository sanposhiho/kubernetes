...
func (pl *TestPlugin) PreBindPreFlight(ctx context.Context, state fwk.CycleState, p *v1.Pod, nodeName string) *framework.Status {
	return framework.NewStatus(framework.Code(pl.inj.PreBindPreFlightStatus), injectReason)
}
...
func TestPreBindPreFlightPlugins(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			plugins: []*TestPlugin{
				{
					name: "TestPlugin1",
					inj:  injectedResult{PreBindPreFlightStatus: int(framework.Skip)},
				},
				{
					name: "TestPlugin2",
					inj:  injectedResult{PreBindPreFlightStatus: int(framework.Success)},
				},
			},
			wantStatus: nil,
		},
		...
		},
		{
			...
			plugins: []*TestPlugin{
				...
				{
					name: "TestPlugin2",
					inj:  injectedResult{PreBindPreFlightStatus: int(framework.Error)},
				},
			},
			wantStatus: framework.AsStatus(fmt.Errorf(`running PreBindPreFlight "TestPlugin2": %w`, errInjectedStatus)),
		},
		{
			...
			plugins: []*TestPlugin{
				{
					name: "TestPlugin",
					inj:  injectedResult{PreBindPreFlightStatus: int(framework.Unschedulable)},
				},
			},
			wantStatus: framework.NewStatus(framework.Error, "PreBindPreFlight TestPlugin returned Unschedulable, which is unsupported. It is supposed to return Success, Skip, or Error status"),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			status := f.RunPreBindPreFlightPlugins(ctx, state, pod, "")
