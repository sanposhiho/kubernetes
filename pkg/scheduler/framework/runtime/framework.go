...
func (f *frameworkImpl) RunPreBindPreFlightPlugins(ctx context.Context, state fwk.CycleState, pod *v1.Pod, nodeName string) (status *framework.Status) {
	startTime := time.Now()
	defer func() {
		metrics.FrameworkExtensionPointDuration.WithLabelValues(metrics.PreBind, status.Code().String(), f.profileName).Observe(metrics.SinceInSeconds(startTime))
	}()
	...
	skipPlugins := sets.New[string]()
	returningStatus := framework.NewStatus(framework.Skip)
	for _, pl := range f.preBindPlugins {
		ctx := ctx
		if verboseLogs {
			logger := klog.LoggerWithName(logger, pl.Name())
		...
		case status.Code() == framework.Error:
			err := status.AsError()
			logger.Error(err, "Plugin failed", "plugin", pl.Name(), "pod", klog.KObj(pod), "node", nodeName)
			return framework.AsStatus(fmt.Errorf("running PreBindPreFlight %q: %w", pl.Name(), err))
		...
		default:
			...
			return framework.AsStatus(fmt.Errorf("PreBindPreFlight %s returned %s, which is unsupported. It is supposed to return Success, Skip, or Error status", pl.Name(), status.Code().String()))
		}
	}
...
func (f *frameworkImpl) runPreBindPreFlightPlugin(ctx context.Context, pl framework.PreBindPlugin, state fwk.CycleState, pod *v1.Pod, nodeName string) *framework.Status {
	if !state.ShouldRecordPluginMetrics() {
