...
func (f *frameworkImpl) fillEnqueueExtensions(p framework.Plugin) {
	...
	if !ok {
		...
		f.enqueueExtensions = append(f.enqueueExtensions, &defaultEnqueueExtensions{pluginName: p.Name()})
		return
	}
...
var _ framework.EnqueueExtensions = &defaultEnqueueExtensions{}

type defaultEnqueueExtensions struct {
	pluginName string
}

func (p *defaultEnqueueExtensions) Name() string { return p.pluginName }
func (p *defaultEnqueueExtensions) EventsToRegister() []framework.ClusterEventWithHint {
	return allClusterEvents
}
