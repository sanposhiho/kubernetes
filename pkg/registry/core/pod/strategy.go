...
func mutatePodTopologySpread(pod *api.Pod) {
	...
	for i := range pod.Spec.TopologySpreadConstraints {
		if pod.Spec.TopologySpreadConstraints[i].LabelSelector == nil {
			// If LabelSelector is nil, we don't need to apply match label keys because nil-LabelSelector is match none.
			continue
		}
		applyLabelKeysToLabelSelector(pod.Spec.TopologySpreadConstraints[i].LabelSelector, pod.Spec.TopologySpreadConstraints[i].MatchLabelKeys, metav1.LabelSelectorOpIn, pod.Labels)
	}
}
