	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.wantPod.Spec.Affinity, pod.Spec.Affinity); diff != "" {
				t.Errorf("%s unexpected affinity (-want, +got): %s", tc.name, diff)
			}
		})
	}
}

func Test_mutatePodTopologySpread(t *testing.T) {
	tests := []struct {
		name               string
		pod                *api.Pod
		wantPod            *api.Pod
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			defer featuregatetesting.SetFeatureGateDuringTest(t, utilfeature.DefaultFeatureGate, features.MatchLabelKeysInPodTopologySpread, tc.featureGateEnabled)()

			pod := tc.pod
			mutatePodTopologySpread(pod)
			if diff := cmp.Diff(tc.wantPod.Spec.TopologySpreadConstraints, pod.Spec.TopologySpreadConstraints); diff != "" {
				t.Errorf("%s unexpected topology spread constraints (-want, +got): %s", tc.name, diff)
			}
		})
	}
}
